...
% feature options, and for HEP: the epoch window (fixed or adaptive), the
% baseline correction, HRSP/HEPC, the surrogate control and, with PPG, the
% pulse arrival time. GUI values are then converted to the formats expected by
...
uilist = {
    {'style' 'text' 'string' 'Analysis to run:' 'fontweight' 'bold' } {'style' 'popupmenu' 'string' analysis_types 'tag' 'analysis' 'callback' mode} ...
    {} ...
    {'style' 'text' 'string' 'Heart data type' 'fontweight' 'bold'} {'style' 'popupmenu' 'string' heart_types 'tag' 'heart_signal' 'callback' heartsig 'value' 1} ...
    {} ...
...
params.heart_signal = lower(heart_types{params.heart_signal});
...
% options for HEP
hepwin = {'Fixed (default)' 'Adaptive to heart rate (within-subject only)'};
hepbl = {'None (default)' 'Regression (Alday, 2019)'};

% options for ECG preprocessing
...
% Callback functions for ECG/PPG preprocessing
...
% Callback functions for EEG & HRV features
...
if strcmp(params.analysis, 'hep') && strcmp(params.heart_signal, 'ecg')    
    ...
    uilist = { ...
        ...
        {} {'style' 'text' 'string' 'Re-reference data to:' } {'style' 'popupmenu' 'string' refmethod 'tag' 'ref' 'enable' 'on' 'userdata' 'clean_eeg' }  ...
        ...
        {'style' 'text' 'string' 'HEP options' 'fontweight' 'bold'} ...
        {} {'style' 'text' 'string' 'Epoch window (ms):' } {'style' 'popupmenu' 'string' hepwin 'tag' 'hep_window_mode'} {'style' 'edit' 'string' '[-300 600]' 'tag' 'hep_window'} ...
        {} {'style' 'text' 'string' 'Baseline correction (window in ms):' } {'style' 'popupmenu' 'string' hepbl 'tag' 'hep_baseline'} {'style' 'edit' 'string' '[-300 -100]' 'tag' 'hep_baseline_win'} ...
        {} {'style' 'checkbox' 'string' 'HRSP and HEPC on all channels (frequency range in Hz):' 'tag' 'hep_tf' 'value' 0} {'style' 'edit' 'string' '[4 30]' 'tag' 'hep_tf_freqs'} ...
        {} {'style' 'text' 'string' 'Surrogate heartbeat control (number of surrogates, 0 = none):' } {'style' 'edit' 'string' '0' 'tag' 'hep_surrogates'} ...
...
elseif strcmp(params.analysis, 'hep') && strcmp(params.heart_signal, 'ppg')
    ...
    uilist = { ...
        ...
        {} {'style' 'checkbox' 'string' 'HRSP and HEPC on all channels (frequency range in Hz):' 'tag' 'hep_tf' 'value' 0} {'style' 'edit' 'string' '[4 30]' 'tag' 'hep_tf_freqs'} ...
        {} {'style' 'text' 'string' 'Surrogate heartbeat control (number of surrogates, 0 = none):' } {'style' 'edit' 'string' '0' 'tag' 'hep_surrogates'} ...
        {} {'style' 'text' 'string' 'Pulse arrival time: ECG channel or delay in ms (empty = none):' } {'style' 'edit' 'string' '' 'tag' 'ppg_transit'} ...
        {'style' 'checkbox' 'string' 'Visualize preprocessings' 'tag' 'vis_cleaning' 'fontweight' 'bold' 'value' 1}  ...
        {'style' 'checkbox' 'string' 'Keep heart channel in final output' 'tag' 'keep_heart' 'fontweight' 'bold' 'value' 0} ...
        };
    uigeom = {
...
elseif strcmp(params.analysis, 'features') && strcmp(params.heart_signal, 'ecg')
    ...
    uilist = { ...
        ...
        {} {'style' 'checkbox' 'string' ' Time domain (rms, kurtosis, mode, skewness, iqr)' 'tag' 'eeg_time' 'value' 1 'userdata' 'eeg_features'} {} ...
        ...
        {} {'style' 'checkbox' 'string' ' Nonlinear domain (entropy, fractal dimension)' 'tag' 'eeg_nonlinear' 'value' 1 'userdata' 'eeg_features'} {} ...
        {} ...
...
if isfield(params, 'hep_window_mode')
    if params.hep_window_mode == 2
        ...
        params.hep_window = str2num(params.hep_window); %#ok<ST2NM>
...
if isfield(params, 'hep_baseline') && isnumeric(params.hep_baseline)
    if params.hep_baseline == 2
        params.hep_baseline = 'regression';
        params.hep_baseline_win = str2num(params.hep_baseline_win); %#ok<ST2NM>
    else
        params.hep_baseline = 'none';
        params = rmfield(params, 'hep_baseline_win');
    end
end
if isfield(params, 'hep_tf')
    params.hep_tf = logical(params.hep_tf);
    params.hep_tf_freqs = str2num(params.hep_tf_freqs); %#ok<ST2NM>
end
if isfield(params, 'hep_surrogates')
    params.hep_surrogates = str2double(params.hep_surrogates);
    if isnan(params.hep_surrogates) || params.hep_surrogates < 0
        error('The number of surrogates must be 0 (none) or more.')
    end
end
if isfield(params, 'ppg_transit')
    v = strtrim(params.ppg_transit);
    if isempty(v)
        params = rmfield(params, 'ppg_transit');
    elseif ~isnan(str2double(v))
        params.ppg_transit = str2double(v);   % delay in ms
    else
        params.ppg_transit = v;               % label of an ECG channel
    end
end
